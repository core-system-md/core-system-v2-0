...
export default function HotSwapSuggestion() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const { data, error: rpcError } = await rpcClient().rpc('get_reception_hot_swap_suggestions_for_pin_session', {
      ...
      setError(err instanceof Error ? err.message : 'تعذر فحص الغرف المتاحة')
      setSuggestion(null)
    } finally {
      setLoading(false)
    }
    ...
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm" dir="rtl" aria-live="polite">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <div className="rounded-xl bg-emerald-500 p-2.5 text-white"><ArrowLeftRight className="h-5 w-5" /></div>
          <div><h3 className="font-bold text-emerald-900">اقتراح تبديل سريع</h3><p className="mt-1 text-xs text-emerald-700">عند وجود غرفة استشارة متاحة وموعد انتظار نشط، يظهر الاقتراح هنا.</p></div>
        </div>
        <button type="button" onClick={() => void load()} disabled={loading} className="rounded-lg border border-emerald-200 bg-white p-2 text-emerald-700 hover:bg-emerald-100 disabled:opacity-50" aria-label="تحديث الاقتراح"><RefreshCw className="h-4 w-4" /></button>
      </div>
