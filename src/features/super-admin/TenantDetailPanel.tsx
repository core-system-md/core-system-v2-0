...
type Tenant = Tables<'master_tenants'>['Row'];
...
export default function TenantDetailPanel({ tenant, isOpen, onClose }: TenantDetailPanelProps) {
  ...
  function renderSettings(settings: unknown): React.ReactNode {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" dir="rtl">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden m-4 flex flex-col">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <InfoCard
                  icon={Building2}
                  label="اسم العيادة (EN)"
                  value={tenant.clinic_name || '—'}
                />
                <InfoCard
                  icon={Phone}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InfoCard
                  icon={CreditCard}
                  label="مفتاح الترخيص"
                  value={tenant.license_key || '—'}
                  monospace
                />
                <InfoCard
                  icon={Tag}
                  label="الخطة"
                  value={TIER_LABELS[tenant.subscription_tier] || tenant.subscription_tier}
                />
                <InfoCard
                  icon={Calendar}
                  label="بداية الاشتراك"
                  value={formatDate(tenant.subscription_start)}
                />
                <InfoCard
                  icon={Calendar}
                  label="نهاية الاشتراك"
                  value={formatDate(tenant.subscription_end)}
                />
                <InfoCard
                  icon={Calendar}
                  label="بداية التجربة"
                  value={formatDate(tenant.trial_started_at)}
                />
                <InfoCard
                  icon={CreditCard}
                  label="العملة"
                  value={tenant.currency ? `${tenant.currency} (${tenant.currency_subunit} وحدة فرعية)` : '—'}
                />
              </div>
            </div>
          )}
          ...
          {activeTab === 'limits' && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <LimitCard
                  icon={Smartphone}
                  label="الأجهزة القصوى"
                  value={tenant.max_devices ?? '—'}
                />
                <LimitCard
                  icon={Users}
                  label="المستخدمون القصوى"
                  value={tenant.max_users}
                />
                <LimitCard
                  label="المرضى القصوى"
                  value={tenant.max_patients}
                />
                <LimitCard
                  label="الإجراءات/الشهر"
                  value={tenant.max_procedures_per_month}
                />
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InfoCard
                  ...
                  colorSwatch={tenant.primary_color}
                />
                <div className="md:col-span-2">
                  ...
                  {tenant.logo_url ? (
                    ...
                    <div className="mt-2 p-4 bg-gray-50 rounded-lg border border-gray-200 text-gray-400 text-sm text-center">
                      لا يوجد شعار
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    {renderSettings(tenant.settings)}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-3 border-t border-gray-200 bg-slate-50 flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 bg-[#1B2A4A] text-white text-sm font-medium rounded-lg hover:bg-[#2a3d66] transition-colors"
          >
            إغلاق
          </button>
        </div>
  ...
  return (
    <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={`text-sm text-gray-800 ${monospace ? 'font-mono' : ''}`}>
          {value ?? '—'}
        </span>
      </div>
    </div>
