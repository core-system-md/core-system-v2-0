...
export default function SurveyRouter() {
  ...
  const handlePage1Next = async (data: NonNullable<SurveyFormData['page1']>) => {
    if (!(await savePage(1, data))) return;
    setFormData((prev) => ({ ...prev, page1: data }));
    setCurrentPage(2);
  };

  const handlePage2Next = async (data: NonNullable<SurveyFormData['page2']>) => {
    if (!(await savePage(2, data))) return;
    setFormData((prev) => ({ ...prev, page2: data }));
    setCurrentPage(3);
  };

  const handlePage3Next = async (data: NonNullable<SurveyFormData['page3']>) => {
    if (!(await savePage(3, data))) return;
    setFormData((prev) => ({ ...prev, page3: data }));
    setCurrentPage(4);
  };

  const handlePage4Next = async (data: NonNullable<SurveyFormData['page4']>) => {
    if (!(await savePage(4, data))) return;
    setFormData((prev) => ({ ...prev, page4: data }));
    setCurrentPage(5);
  };

  const handlePage5Complete = async (data: NonNullable<SurveyFormData['page5']>) => {
    if (!(await savePage(5, data))) return;
    setFormData((prev) => ({ ...prev, page5: data }));
    setCompleted(true);
  };

  const handleBack = (page: SurveyPage) => {
    setError(null);
    setCurrentPage(page);
  };

  if (completed) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4" dir="rtl"><Card className="max-w-md border-emerald-200 shadow-sm"><CardContent className="pt-8 pb-8 text-center space-y-4"><CheckCircle2 className="h-14 w-14 text-emerald-600 mx-auto" /><h1 className="text-xl font-bold text-slate-900">تم إكمال الاستبيان بنجاح</h1><p className="text-sm text-slate-600 leading-6">تم حفظ إجاباتك وتوقيعك الإلكتروني بنجاح. شكراً لوقتك.</p></CardContent></Card></div>;
  }
