...
const tabs: { id: TabId; label: string; icon: typeof LayoutDashboard; path: string }[] = [
  ...
  { id: 'inventory', label: 'المخزون', icon: Package, path: '/admin/inventory' },
...
export default function AdminLayout() {
  ...
  useEffect(() => { if (!isAuthenticated) { navigate('/login'); return; } if (role !== 'clinic_admin' && role !== 'super_admin') navigate('/doctor'); }, [isAuthenticated, role, navigate]);
  if (!isAuthenticated || (role !== 'clinic_admin' && role !== 'super_admin')) return null;
  const activeTab = tabs.find((t) => location.pathname === t.path)?.id || 'overview'; const roleLabel = role ? ROLE_LABELS_AR[role] : 'مدير العيادة'; const displayName = user?.full_name_ar || user?.full_name || roleLabel;
  return <div className="min-h-screen bg-background" dir="rtl"><div className="max-w-7xl mx-auto p-4 space-y-4"><div className="flex items-center justify-between"><h1 className="text-2xl font-bold text-primary">لوحة تحكم العيادة</h1><p className="text-muted-foreground">مرحباً {displayName}</p></div><div className="w-full overflow-x-auto"><div className="flex min-w-max gap-1 bg-muted rounded-lg p-1">{tabs.map((tab) => <button key={tab.id} onClick={() => navigate(tab.path)} className={`flex items-center justify-center gap-2 py-2 px-4 rounded-md text-sm font-medium transition-all ${activeTab === tab.id ? 'bg-white text-primary shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><tab.icon className="w-4 h-4" />{tab.label}</button>)}</div></div><div className="mt-4"><Outlet /></div></div></div>;
}
