...
const navClass = (active: boolean) =>
  `inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition-colors ${
    active ? 'bg-[#1B2A4A] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
  }`;
...
export default function DoctorLayout() {
  ...
  useEffect(() => {
    ...
    if (role !== 'doctor' && role !== 'clinic_admin' && role !== 'super_admin') {
      navigate('/reception', { replace: true });
    }
  }, [isAuthenticated, role, navigate]);

  if (!isAuthenticated || (role !== 'doctor' && role !== 'clinic_admin' && role !== 'super_admin')) {
    return null;
  }

  const onTodayPatients = () => {
    if (location.pathname !== '/doctor') navigate('/doctor');
  };
      ...
      <div className="min-h-screen bg-slate-50" dir="rtl">
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
          <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 md:px-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h1 className="text-base font-bold text-[#1B2A4A]">محطة الطبيب</h1>
                </div>
              </div>
            ...
            <nav className="flex flex-wrap gap-2 overflow-x-auto pb-1" aria-label="واجهة الطبيب">
              <PermissionGuard required="view_sessions">
                <button
                  ...
                  className={navClass(location.pathname === '/doctor')}
            ...
            <div className="hidden items-center gap-3 rounded-xl bg-slate-50 px-3 py-2 lg:flex">
