...
type PinSessionRpc = (args: { p_tenant_id: string; p_pin: string }) => Promise<{ data: unknown; error: { message: string } | null }>;
...
export function useAuth() {
  ...
  const validateLicense = useCallback(async (key?: string) => {
    ...
    try {
      ...
      store.setTenant(String(tenant.id), { clinicName: (tenant.clinic_name as string | null) || null, subscriptionTier: (tenant.subscription_tier as string) || 'trial', primaryColor: (tenant.primary_color as string | null) || '#1B2A4A', logoUrl: (tenant.logo_url as string | null) || null });
      return { success: true, tenant_id: String(tenant.id) };
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'License validation failed'; store.setError(msg); return { success: false, error: msg };
    }
  }, [store]);

  const loginWithPin = useCallback(async (pin: string) => {
    if (!/^\d{4}$/.test(pin)) { store.setError('PIN must be exactly 4 digits'); return { success: false, error: 'PIN must be exactly 4 digits' }; }
  ...
  const logout = useCallback(async () => { sessionStorage.removeItem(PIN_SESSION_STORAGE_KEY); await store.logout(); }, [store]);
