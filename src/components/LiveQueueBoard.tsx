...
export function LiveQueueBoard({ onSelectSession }: LiveQueueBoardProps) {
  ...
  return <div className="flex h-full flex-col" dir="rtl">
    <header className="flex items-center justify-between border-b bg-white px-4 py-3"><div><h2 className="text-lg font-semibold text-[#1B2A4A]">الانتظار المباشر</h2><p className="text-xs text-slate-500">السحب والإفلات لإعادة ترتيب المرضى</p></div><div className="flex items-center gap-3"><div className="flex items-center gap-1.5 text-sm"><span className="h-2 w-2 rounded-full bg-emerald-500" /><span className="text-slate-500">{getActiveCount()} بالانتظار</span></div>{getRedCount() > 0 && <div className="flex items-center gap-1.5 text-sm text-red-600"><span className="h-2 w-2 animate-pulse rounded-full bg-red-500" />{getRedCount()} تجاوز SLA</div>}</div></header>
    <div className="flex-1 overflow-y-auto p-4">{isLoading ? <div className="space-y-3">{[1,2,3].map((i) => <div key={i} className="h-24 animate-pulse rounded-lg bg-slate-100" />)}</div> : items.length === 0 ? <div className="flex h-full flex-col items-center justify-center py-12 text-center"><p className="text-slate-500">لا يوجد مرضى في الانتظار</p><p className="mt-1 text-sm text-slate-400">القائمة فارغة حاليًا</p></div> : <div className="space-y-3">{items.map((item,index) => { const isWaiting=item.sessionStatus==='waiting'; const lockedByOther=!!item.lockHolderId; return <div key={item.sessionId} className={cn('rounded-2xl border bg-white p-4 transition-all',selectedSessionId===item.sessionId?'border-[#1B2A4A] bg-blue-50/40':'border-slate-200',draggedId===item.sessionId&&'opacity-60',busyId===item.sessionId&&'pointer-events-none opacity-60')}>
      <div className="flex items-start gap-3">{isWaiting ? <PermissionGuard required="edit_queue" fallback={<div className="pt-1 text-slate-300"><GripVertical className="h-5 w-5" /></div>}><button type="button" draggable={!lockedByOther} onDragStart={()=>setDraggedId(item.sessionId)} onDragEnd={()=>setDraggedId(null)} onDragOver={(event)=>event.preventDefault()} onDrop={()=>void handleDrop(item.sessionId)} className="cursor-grab pt-1 text-slate-400" aria-label="إعادة ترتيب"><GripVertical className="h-5 w-5" /></button></PermissionGuard> : <div className="pt-1 text-slate-200"><GripVertical className="h-5 w-5" /></div>}
        <button type="button" onClick={()=>handleSelect(item.sessionId)} className="min-w-0 flex-1 text-right"><div className="flex items-center justify-between gap-4"><div className="flex min-w-0 items-center gap-2"><span className="truncate font-semibold text-slate-800">{item.patientName}</span><PriorityBadge priority={item.priority}/></div><SlaIndicator status={item.slaStatus} waitMinutes={item.waitMinutes}/></div>{item.procedureName&&<p className="mt-1 truncate text-sm text-slate-500">{item.procedureName}</p>}</button>
      ...
      </div><div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2 text-xs text-slate-400"><span>الموقع {index+1}</span>{item.coreScoreDisplay!==null&&<span className="font-mono">Core Score {item.coreScoreDisplay}</span>}</div>
    </div>})}</div>}</div>
  </div>
}
