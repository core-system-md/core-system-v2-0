-- CORE SYSTEM v2.1 — Doctor PIN-session read boundary
-- Evidence: the active PIN authentication flow has no Supabase Auth JWT.
-- DoctorSessionView therefore cannot safely read clinic_visit_sessions directly:
-- RLS returns no rows and .single() becomes PGRST116/406.
-- This RPC is the narrow server-side read boundary for that screen.
