...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'patient_intake_responses_visit_type_selection_check'
  ) THEN
    ALTER TABLE public.patient_intake_responses
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'patient_intake_responses_readiness_level_check'
  ) THEN
    ALTER TABLE public.patient_intake_responses
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'patient_intake_responses_followup_importance_check'
  ) THEN
    ALTER TABLE public.patient_intake_responses
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'patient_intake_responses_openness_to_proceed_check'
  ) THEN
    ALTER TABLE public.patient_intake_responses
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'patient_intake_responses_completion_status_check'
  ) THEN
    ALTER TABLE public.patient_intake_responses
