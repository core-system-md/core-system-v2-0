-- P136: Analytics snapshot must honor the established soft-delete contract for inquiries.
-- No schema/RLS/RPC signature change; only the existing RPC body is reconciled.
...
BEGIN
  ...
  session_metrics AS (
    ...
    LEFT JOIN public.clinic_patients p ON p.id = ds.patient_id AND p.deleted_at IS NULL
