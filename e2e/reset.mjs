...
if (process.env.E2E_ALLOW_PRODUCTION && !/^https:\/\//i.test(process.env.E2E_BASE_URL ?? '')) {
  throw new Error('[E2E] E2E_ALLOW_PRODUCTION is only valid with an explicit HTTPS target.');
}
...
const { error: intakeError } = await supabase
  ...
  .like('session_id', '10000000-0000-4000-8000-%')
  .is('deleted_at', null);
