...
async function loginAs(page, staff) {
  ...
  try {
    await expectPath(page, defaultRoute[staff.role]);
  } catch (error) {
...
test.describe('security negative browser suite', () => {
  ...
  for (const staff of E2E_STAFF) {
    test(`${staff.role}: denied privileged routes redirect to role default`, async ({ page }) => {
      const deniedRoutes = privilegedDenied[staff.role] ?? [];
      if (deniedRoutes.length === 0) return;
      ...
      for (const route of deniedRoutes) {
        ...
        try {
          ...
          throw new Error(`Denied-route redirect failed: ${JSON.stringify(diagnostics)}\n${error instanceof Error ? error.message : String(error)}`);
        }
      }
    });
  }
